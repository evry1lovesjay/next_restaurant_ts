      ...
      <table className="w-full border-separate border-spacing-3">
        ...
        {/* text-sm md:text-base */}
          {data.map((item: OrderType) => (
            <tr className={`${item.status !== "delivered" ? "bg-red-200" : "bg-green-300"} text-sm md:text-base`} key={item._id}>
            <td className="hidden md:block py-6 px-1">{item._id}</td>
            <td className="py-6 px-1">{item.createdAt.toString().slice(0, 10)}</td>
            <td className="hidden md:block py-6 px-1">{item.price}</td>
              ...
              (
                <td>
                  <form className="flex items-center justify-center gap-4" onSubmit={(e)=>handleUpdate(e, item._id)}>
                    <input placeholder={item.status} className="p-2 ring-1 ring-red-100 rounded" />
